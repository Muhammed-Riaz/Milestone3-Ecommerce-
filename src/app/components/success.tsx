...
// Success, Error, and Info icons for visual feedback
const SuccessIcon = (
...
// Define a union type for payment statuses
type PaymentStatus = "succeeded" | "processing" | "requires_payment_method" | "default";

// Status content mapping for various payment states
...
export default function CompletePage() {
  ...
  React.useEffect(() => {
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      ...
      // Update the state with the payment status
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 px-4 py-8">
      ...
      <h2 className="text-2xl font-semibold text-center mb-6">{STATUS_CONTENT_MAP[status].text}</h2>

      {intentId && (
        <div className="bg-white shadow-lg rounded-lg p-6 mb-4 w-full max-w-md">
          <table className="w-full">
            <tbody>
              <tr className="flex sm:block flex-col  justify-start w-full">
                <td className="font-semibold text-gray-600">ID</td>
                <td className="text-gray-900 flex text-xs  sm:text-lg ">{intentId}</td>
              </tr>
              <tr className="flex sm:block flex-col">
                <td className="font-semibold flex text-gray-600">Status</td>
                <td className="text-gray-900">{status}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <a
        ...
        className="mt-6 text-lg text-blue-600 hover:text-blue-800 font-semibold"
      >
       Check Ship address
      </a>
